            ...
            <tr>
                ...
                <td class="align-middle">
                    <div class="d-flex align-items-center">
                        @can('read-role')
                            <div class="me-3">
                                <x-table.edit-btn :label="__('messages.table.actions.view')" :href="route('roles.show', ['role' => $model])" />
                            </div>
                        @endcan
                        @can('update-role')
                            <div class="me-3">
                                <x-table.edit-btn :label="__('messages.table.actions.edit')" :href="route('roles.edit', ['role' => $model])" />
                            </div>
                        @endcan
                        @can('delete-role')
                            <div>
                                <x-table.delete-btn :label="__('messages.table.actions.delete')" :id="$model->id" :action="route('roles.destroy', $model)"
